...
public class WeatherGenerator {
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        if (dayOne < 0.5) {
            forecast[0] = WET; //first day is wet
        } else {
            forecast[0] = DRY; //first day is dry
        }
        int count = 1;
        double probability = 0.0;
        while (count < numberOfDays) {
            double day = StdRandom.uniform();
            if (forecast[count - 1] == WET) {
                probability = wetwetProbability;
            } else {
                probability = drywetProbability;
            }
            if (day < probability) {
                forecast[count] = WET;
            } else {
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        double drywetProbs = drywetProbability[month];
        double wetwetProbs = wetwetProbability[month];
        int days = numberOfDaysInMonth[month];
        return forecastGenerator(drywetProbs, wetwetProbs, days);
    }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for (int i = 0; i < forecast.length; i++) {
            if (forecast[i] == mode) {
                ...
                if (count > highest) { //if current streak is longer than current highest
                    highest = count;
                }
                count = 0;
            }
    ...
    public static int bestWeekToTravel(int[] forecast){
        int sum = forecast[0] + forecast[1] + forecast[2] + forecast[3] + forecast[4] + forecast[5] + forecast[6];
        int highest = sum;
        int index = 0;
        for (int i = 1; i < forecast.length - 6; i++) {
            sum = sum - forecast[i - 1] + forecast[i + 6];
            if (sum > highest) {
                highest = sum;
                index = i;
            }
